...
#todo egg characters (segunda solapa por ej) con un enable/disable de todos, cambiar genero, asignar male/Female a slots, desbloquear todos los warp points, desbloquear   resetear flags de historia y cofres/leafs/items recogidos
...
class MagicalEditor:
    ...
    def __init__(self, root):
        ...
        self.fileSelected = None #1, 2 o 3
        ...
        root.rowconfigure(0, weight=1)
            ...
            #Boton Open .sav
        row = 12
        tk.Button(mainframe, text='Open save file', command=self.open_sav).grid(column = 2, row = row, sticky = S)
        ttk.Label(mainframe, textvariable=self.savPath).grid(column=3, row=row+1, columnspan=10, sticky=(W, E))

            #Boton Export .sav
        row = 12
        tk.Button(mainframe, text='Export save file', command=self.export_sav).grid(column = 3, row = row, sticky = S)

        #Botones desbloquear cosas
        botonBestiario = tk.Button(mainframe, text= "Unlock Bestiary", command= self.desbloquearBestiario).grid(column = 6, row = row, sticky = (N, S))
        botonMapaEnciclopediaDiario = tk.Button(mainframe, text= "Unlock Maps/Diary/Encl", command= self.desbloquearMapas).grid(column = 7, row = row, sticky = (N, S)) #todo imagen
        botonEggs = tk.Button(mainframe, text= "Unlock Egg Characters", command= self.desbloquearHuevos).grid(column = 8, row = row, sticky = (N, S))
        ...
        for i in range(4):
            ...
            label.grid(column = 6+i, row = row+1)
            self.labelsDone.append(label)

        #File Select
        self.fileSelected = tk.IntVar()
        for i in range(1,4):
            filei = tk.Radiobutton(mainframe, text = 'File '+ str(i), value = i, variable = self.fileSelected)
            filei.grid(column = i, row = 13)
        #todo combobox o algo, de esos con circulito, y luego sumar offsets en Read/Write1
    ...
    def updateCharacterData(self, char):
        ...
        for stat in CHAR_STATS:
            ...
            writeBytes(self.savData, char.offset + stat.offset, val, stat.nBytes)
        ...
        if(char.id>=2): chari = char.id-1
        if(char.id ==0): chari = 0
        writeBytes(self.savData, namesOff+ chari*0x10,nombre_bytes, 8*2)
    ...
    def desbloquearBestiario(self):
        ...
        writeBytes(self.savData, bestiaryOffset, checkAll, bestiarySize)
    ...
    def desbloquearMapas(self):
        ...
        writeBytes(self.savData, enciclopedymapdiaryOffset , checkAll, enciclopedymapdiarySize)
...
def readBytes(file, offset, nBytes):
...
def writeBytes(file, address, val, nBytes, dup_offset = offsetDuplicado):
