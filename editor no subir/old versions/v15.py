...
class MagicalEditor:
    ...
    def __init__(self, root):
        ...
        tk.Label(self.mainFrame, text =' Play Time (hh:mm)').grid(column=11, row=5, sticky=(W,S))
        ...
        #Separador gráfico
        separator = ttk.Separator(self.mainFrame, orient='horizontal')
        separator.grid(row=7, column = 11, sticky = (W, E, N))

        #Bira (v15)
        self.biraActual = tk.IntVar()
        tk.Label(self.mainFrame, text =' Bira').grid(column=11, row=7, sticky=(W, S))
 ...
 self.cambiarSprites).grid(column = column+4, row = row, sticky = (E, N, S), padx = 30)
    ...
    def readload_timeBira(self, *args):
    ...
    def updateSavData(self):
        ...
        self.updateGender()
        self.updatePlayTimeBira()

    def updatePlayTimeBira(self):
        ...
        try:
            ...
            self.writeBytes(0x0C,timeFrames, 4, offsetFile = headerFileOffset)
        except: return # si se puso mal (incompleto) el formato de hora, deja como está

        self.writeBytes(0x10,self.biraActual, 4, offsetFile = headerFileOffset)
