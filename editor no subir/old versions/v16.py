...
class MagicalEditor:
    ...
    def __init__(self, root):
        ...
        for i in range(3):
            ...
            genderButton.grid(column = 11, row = i+1, sticky = (W,N, E) )
        ...
        tk.Label(self.mainFrame, text =' Play Time (hh:mm)').grid(column=11, row=5, sticky=(N, W,S))
        timeBox = ttk.Entry(self.mainFrame, textvariable = self.playTimeActual)
        timeBox.grid(column = 11, row = 6, sticky = (W,N, E) )
        ...
        tk.Label(self.mainFrame, text =' Bira').grid(column=11, row=7, sticky=(N,W, S))
        biraBox = ttk.Entry(self.mainFrame, textvariable = self.biraActual)
        biraBox.grid(column = 11, row = 8, sticky = (W,N, E) )
        ...
        tk.Label(self.mainFrame, text ='Location ID').grid(column=11, row=10, sticky=(N,W, S))
        locationIdBox = ttk.Entry(self.mainFrame, textvariable = self.locationIdActual)
        locationIdBox.grid(column = 11, row = 11, sticky = (W,N, E) )
        validate_command =self.mainFrame.register(only_numeric_input)
        locationIdBox.configure(validate="key",validatecommand=(validate_command,'%P'))  #Que solo permita numeros
        #todo agregar label que te diga qué lugar en concreto es (de maps ids sale)

        #Save File ID
        self.fileId = tk.IntVar()
        tk.Label(self.mainFrame, textvariable = self.fileId).grid(column=11, row=9, sticky=(W, S))
 ...
 self.cambiarSprites).grid(column = column+4, row = row, sticky = (E, N, S), padx = 30)
    ...
    def updateGlobalHeader(self):
        ...
        #Bira
        self.writeMultiBytes2(0x10,self.biraActual.get(), 4, sction = 'header')
    ...
    def writeBytes2(self, address, val, nBytes, section = 'file'):
        ...
        if(section == 'header'):
            self.writeBytes(address, val, nBytes, dup_offset = headerDuplicadoOffset, offsetFile = headerFileOffset)
        if(section == 'file'):
